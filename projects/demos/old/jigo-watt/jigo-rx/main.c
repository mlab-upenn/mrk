...
void rx_task ()
{
  ...
  while (1) {
    ...
    rms_current=(uint16_t)local_rx_buf[0]<<8; 
    rms_current|=(uint16_t)local_rx_buf[1]; 
    rms_voltage=(uint16_t)local_rx_buf[2]<<8; 
    rms_voltage|=(uint16_t)local_rx_buf[3];
    freq=local_rx_buf[4];
    true_power=(uint32_t)local_rx_buf[5]<<16;
    true_power|=(uint32_t)local_rx_buf[6]<<8;
    true_power|=(uint32_t)local_rx_buf[7];
    tmp_energy=(uint32_t)local_rx_buf[8]<<24;
    tmp_energy|=(uint32_t)local_rx_buf[9]<<16;
    tmp_energy|=(uint32_t)local_rx_buf[10]<<8;
    tmp_energy|=(uint32_t)local_rx_buf[11];
    l_v_p2p_high=(uint16_t)local_rx_buf[12]<<8;
    l_v_p2p_high|=(uint16_t)local_rx_buf[13];
    l_v_p2p_low=(uint16_t)local_rx_buf[14]<<8;
    l_v_p2p_low|=(uint16_t)local_rx_buf[15];
    l_c_p2p_high=(uint16_t)local_rx_buf[16]<<8;
    l_c_p2p_high|=(uint16_t)local_rx_buf[17];
    l_c_p2p_low=(uint16_t)local_rx_buf[18]<<8;
    l_c_p2p_low|=(uint16_t)local_rx_buf[19];
    total_secs=(uint32_t) local_rx_buf[20]<<24;
    total_secs|=(uint32_t) local_rx_buf[21]<<16;
    total_secs|=(uint32_t) local_rx_buf[22]<<8;
    total_secs|=(uint32_t) local_rx_buf[23];
    nrk_kprintf(PSTR("\r\n******************************") );
    nrk_kprintf(PSTR("\r\ni RMS: ")); printf( "%u",rms_current );
    nrk_kprintf(PSTR("\r\nv RMS: ")); printf( "%u",rms_voltage);
    nrk_kprintf(PSTR("\r\nfreq: ")); printf( "%u",freq);
    nrk_kprintf(PSTR("\r\ntrue power: ")); printf( "%lu",true_power);
    nrk_kprintf(PSTR("\r\nenergy: ")); printf( "%lu",tmp_energy);
    nrk_kprintf(PSTR("\r\nv p2p: ")); printf( "%u,%u",l_v_p2p_low, l_v_p2p_high);
    nrk_kprintf(PSTR("\r\nc p2p: ")); printf( "%u,%u",l_c_p2p_low, l_c_p2p_high);
    nrk_kprintf(PSTR("\r\ntime: ")); printf( "%lu\r\n",total_secs);
