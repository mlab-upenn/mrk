...
void write_ff_env(struct firefly_env ff){
	...
	for(i=0; i<ERROR_RETRIES; i++ ){
		if(!table_exists(db, ff.id)){
			sprintf(cmdbuf,"create table '%s' (time int, light int, temp int, accl int, voltage int, audio int, arb1 int, arb2 int)",ff.id);
		...
		if(ff.arb1 == INT_MIN) sprintf(cmdbuf,"%s NULL)",cmdbuf);
		else sprintf(cmdbuf,"%s %d,",cmdbuf, ff.arb1);

		if(ff.arb2 == INT_MIN) sprintf(cmdbuf,"%s NULL)",cmdbuf);
		else sprintf(cmdbuf,"%s %d)",cmdbuf, ff.arb2);
