...
retry_num=0;
  ...
  if (argc < 4 ) {
    printf ("Usage: server port mac-addr [-t timeout] [-d] [-l] [-r num-retries]\n");
    ...
    printf ("  -r        number of retries for multicast\n");
    exit (1);
  }
  ...
  for(i=0; i<argc; i++ )
	...
	dst_mac=(tmp & 0xff);	
  	...
  	if(debug_txt_flag==1) printf( "FULLMAC: 0x%x\n",tmp);
  	if(debug_txt_flag==1) printf( "Local MAC_ADDR: 0x%x\n",dst_mac);
...
//	print_ds_packet(&ds_pkt );
	...
	if(gw_pkt.pkt_type==TRANSDUCER_PKT )
		...
		if(tran_msg.type==TRAN_FF_BASIC_SHORT )
			{
  ...
  // This takes the structure and packs it into the raw
	...
	if(gw_pkt.pkt_type==TRANSDUCER_PKT )
		...
		for(i=0; i<tran_pkt.num_msgs; i++ )
                {
                transducer_msg_get(&tran_pkt, &tran_msg, i );
		if(tran_msg.type==TRAN_FF_BASIC_SHORT && tran_msg.mac_addr==dst_mac)
			{
			ff_basic_sensor_short_unpack (&tran_msg, &sensor_short);
               	 	printf ("Sensor pkt from 0x%x\n", tran_msg.mac_addr);
                	printf ("   Light: %d\n", sensor_short.light);
