...
retry_num=0;
  ...
  if (argc < 4 ) {
    ...
    printf ("  ex: %s localhost 5001 0xffffffff -r 3\n",argv[0]);
    printf ("  server    Address of SLIPstream server or mirror\n");
    ...
    printf ("  -d        debug mode\n");
    printf ("  -l        enable LED debug for multi-cast\n");
    printf ("  -r        retry count for multi-cast\n");
    printf ("\nExample Usages:\n");
    printf ("Ping request to node 00000a04 with gateway_cleint running\n");
    printf ("  %s localhost 5001 0x00000a04\n",argv[0]);
    printf ("Ping broadcast request to subnet 00000a with 3 retries\n");
    printf ("  %s localhost 5001 0x00000aff -r 3\n",argv[0]);
    printf ("Ping broadcast request to subnet 00000a with LED debugging\n");
    printf ("  %s localhost 5001 0x00000aff -l\n",argv[0]);
    exit (1);
  }
...
//	print_ds_packet(&ds_pkt );
    ...
    while (reply_timeout > time (NULL)) {
      ...
      if (v > 0) {
	status=print_gw_packet_elements(rx_buf, v);
	}
      usleep (1000);
    }
}
...
for(retry_cnt=0; retry_cnt<retry_num; retry_cnt++ )
{
    ...
    while (reply_timeout > time (NULL)) {
      v = slipstream_receive (rx_buf);
      if (v > 0) {
	if (check_for_ack(rx_buf,v, dst_mac, p2p_pkt.seq_num)==1 ) { printf( "ACK from 0x%x\n",dst_mac ); return 1; }
      }
      usleep (1000);
    }
