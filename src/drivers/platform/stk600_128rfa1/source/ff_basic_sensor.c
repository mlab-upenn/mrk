...
uint8_t read(uint8_t action, uint8_t opt, uint8_t *buffer, uint8_t size) {
    ...
    if (channel != BAT && channel != TEMP && channel != PRESS && channel < 7) {
        ...
        uint8_t count = 0;
        value_from_sensor = get_humidity();
        buffer[count] = value_from_sensor & 0xFF;
        count++;
        buffer[count] = (value_from_sensor >> 8) & 0xFF;
    }
...
uint16_t buf;
        ...
        //printf("Timer fired with val %lu\r\n", nrk_timer_int_read(NRK_APP_TIMER_0));
}

uint8_t get_humidity() {

    uint8_t val; // Used for getting statuses and values
    uint16_t timer_val = 0; // Value of the timer
    uint8_t counter = 0;
    uint16_t buf[NUMBER_HUM_SAMPLES]; // Used for holding the recording period times
    uint16_t mode; // The mode of the recorded sample values
    uint16_t humidity; // The value of the humidity to be returned

    // Set up the timer with the callback. Note that the callback function should
    // never actually get called.
    val=nrk_timer_int_configure(NRK_APP_TIMER_0, 3, 1000000, &timer_callback );

    // Set up the nrk_gpio
    val = nrk_gpio_direction(0x15, NRK_PIN_INPUT);
    ...
    if (val == NRK_ERROR) nrk_kprintf(PSTR("Failed to set gpio direction.\r\n"));

    val = nrk_timer_int_start(NRK_APP_TIMER_0);
    if (val == NRK_ERROR) nrk_kprintf(PSTR("Failed to start timer.\r\n"));

    val = nrk_timer_int_stop(NRK_APP_TIMER_0);
    if (val == NRK_ERROR) nrk_kprintf(PSTR("Failed to stop timer.\r\n"));

    // get samples
    while (counter < NUMBER_HUM_SAMPLES) {

        wait_low();
        wait_high();
        val = nrk_timer_int_read(NRK_APP_TIMER_0);
        buf[counter] = val;

        val = nrk_timer_int_reset(NRK_APP_TIMER_0);
        if (val == NRK_ERROR) nrk_kprintf(PSTR("Failed to reset timer.\r\n"));
        counter++;
    }

    /*
    ** Due to the fairly frequent spikes in the recorded values, the best value
    ** to use is the mode, rather than the mean or median. 
    */
    mode = calculate_mode(buf);

    // This "magic" formula was determined from empirical testing with the sensor;
    humidity = 2.9238 * (float) mode - 271.59;

    // It is hard to test this but to not freak out the user, we just return 100
    if(humidity > 100)
        humidity = 100;
...
uint16_t calculate_mode(uint16_t buf[]){
    ...
    uint16_t mode_buf[NUMBER_HUM_SAMPLES][2];

    for(i = 0; i < NUMBER_HUM_SAMPLES; i++)
        for(j = 0; j < 2; j++)
            mode_buf[i][j] = 0;

    // This is going to be slow
    for(i = 0; i < NUMBER_HUM_SAMPLES; i++){ // Iterate over all the samples in the buf

        uint16_t current_val = buf[i];

        // Check to see if this number is already in the mode_buf
        for(j = 0; j < NUMBER_HUM_SAMPLES; j++){
            if(mode_buf[j][0] == current_val){
                mode_buf[j][1]++;
                break;
            }
            else if(mode_buf[j][0] == 0){
                mode_buf[j][0] = current_val;
                mode_buf[j][1]++;
                break;
            }
        }
    ...
    // Now iterate over the mode_buf and find the most common value
    uint16_t highest_count_found = 0;
    uint16_t index_of_highest_count_found = 0;
    for(i = 0; i < NUMBER_HUM_SAMPLES; i++){
        if(mode_buf[i][1] > highest_count_found){
            highest_count_found = mode_buf[i][1];
            index_of_highest_count_found = i;
        }
    }

    uint16_t mode = mode_buf[index_of_highest_count_found][0];
...
uint8_t set_status(uint8_t action, uint8_t opt, uint8_t *buffer, uint8_t size) {
    ...
    if (key == SENSOR_SELECT) {
        ...
        if (value == AUDIO_P2P) {
            ...
            if (value > 10) { // Update for all of the sensors
                _nrk_errno_set(1);
                return NRK_ERROR;
            }
            channel = value;
...
int32_t calc_true_press(uint8_t oss) {
    ...
    B6 = B5 - 4000;
    if (DEBUG) printf("B6 %li\r\n", B6);
    X1 = (B2 * (B6 * B6 / pow(2, 12))) / pow(2, 11);
    if (DEBUG) printf("X1 %li\r\n", X1);
    X2 = AC2 * B6 / pow(2, 11);
    if (DEBUG) printf("X2 %li\r\n", X2);
    X3 = X1 + X2;
    if (DEBUG) printf("X3 %li\r\n", X3);
    B3 = (((AC1 * 4 + X3) << oss) + 2) / 4;
    if (DEBUG) printf("B3 %li\r\n", B3);
    X1 = AC3 * B6 / pow(2, 13);
    if (DEBUG) printf("X1 %li\r\n", X1);
    ...
    if (DEBUG) printf("B4 %lu\r\n", B4);
    B7 = (UP - B3) * (50000 >> oss);
    if (DEBUG) printf("B7 %lu\r\n", B7);
