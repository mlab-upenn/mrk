...
// TICK-INDEPENDENT TIMES
    ...
    //#define TDMA_RX_WAIT_MS       		1 //shooting for 2
...
// how often to send sync packets (1 is every cycle, 2 is every other cycle, etc)
#define SYNC_STRIDE 1
...
// Note: The times such as delta slots don't matter for this because we're using the high speed timer to time these.
    ...
    // slot stealing and slot size parameters

    // how big a delta slot is, this is just used here to determine the length of the slot.
    //#define TDMA_DELTA_SLOT_US 700
    //#define TDMA_DELTA_SLOT_US 600
    //#define TDMA_DELTA_SLOT_US 500
    //#define TDMA_DELTA_SLOT_US 400
    #define TDMA_DELTA_SLOT_US 300
    
    #define TREE_MAX_CHILDREN 5
    #define TREE_MAX_NEIGHBORS 15 

    #define NUM_DELTA_SLOTS (TREE_MAX_CHILDREN+1)

    // this is where we start, because we need to line up with os tick boundary
    #define TDMA_TICKS_PER_SLOT 12
    #define TDMA_SLOT_TIME_US   11724 // 977 us per tick * 12 ticks per slot

    #define TDMA_DELTA_SLOT_PERIOD_US (NUM_DELTA_SLOTS * TDMA_DELTA_SLOT_US)
    #define TDMA_SLOT_DATA_TIME_US  (TDMA_SLOT_TIME_US - TDMA_DELTA_SLOT_PERIOD_US)

    // TICK VALUES, THESE SHOULD BE DERIVED DIRECTLY FROM THE ABOVE VALUES IF THEY ARE CHANGED
    //#define TDMA_DELTA_SLOT_TICKS 5161 // calculated by TDMA_DELTA_SLOT_US * 7.3728, 600us
    //#define TDMA_DELTA_SLOT_TICKS 4424 // calculated by TDMA_DELTA_SLOT_US * 7.3728, 600us
    //#define TDMA_DELTA_SLOT_TICKS 3686 //  500us
    //#define TDMA_DELTA_SLOT_TICKS 2949 //  400us
    #define TDMA_DELTA_SLOT_TICKS 2212 //  300us
    #define DATA_SLOT_START_TICKS 26544  // calculated by TDMA_DELTA_SLOT_PERIOD_US * 7.3728

    // for 700us deltas
    //#define DATA_SLOT_START_TICKS 30966  // calculated by TDMA_DELTA_SLOT_PERIOD_US * 7.3728

    #define DELTA_SLOT_OFFSET_TICKS 400

    // sfd to slot time, this silly silly thing
    #ifdef TDMA_STEAL_UPSTREAM
        #define SYNC_DELTA_SLOT_TICKS 6000
        #define SFD_TO_SLOT_TIME (44732-SYNC_DELTA_SLOT_TICKS)
    #else
...
// slot stealing for sync slots, if the parent doesn't txsync
//#define TDMA_STEAL_UPSTREAM
