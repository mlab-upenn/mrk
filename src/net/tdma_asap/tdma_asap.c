...
// XXX DANGEROUS FOR REAL USE! IT CHANGES THE VALUE OF THE PACKET JUST BEFORE SENDING
#define TDMA_TIMESTAMP_PACKETS
...
#ifdef TDMA_STEAL_UPSTREAM
    // send a sync packet every 2 cycles
    uint8_t can_steal;
#endif
...
// raise pin for waits and stuff for upstream stealing
#define TDMA_UPSTREAM_DEBUG DEBUG_1
...
tdma_schedule_entry_t cur_slot;
...
void _tdma_rx (uint16_t slot)
{
    ...
    */
    ...
    #ifdef TDMA_STEAL_UPSTREAM
        // If I'm actually a parent who would send a sync packet (but am not)
        // on this slot, I will wait for the 'real' slot to begin,
        // beacuse in the upstream stealing scheme an extra delta slot is
        // prepended to the slot
        if (cur_slot.type == TDMA_TXSYNC)
        {
            while(_nrk_high_speed_timer_get() < SYNC_DELTA_SLOT_TICKS);
            // for all purposes of the rest of the slot, this is now the beginning
            _nrk_high_speed_timer_reset();
    #ifdef TDMA_UPSTREAM_DEBUG`
            // after I've waited, beginning of slot should line up
            PORTA |= BM(TDMA_UPSTREAM_DEBUG);
            PORTA &= ~BM(TDMA_UPSTREAM_DEBUG);
    #endif
        }
    #endif
        ...
        while(_nrk_high_speed_timer_get() < DELTA_SLOT_OFFSET_TICKS);
    ...
    uint16_t timeout = DATA_SLOT_START_TICKS + DELTA_SLOT_OFFSET_TICKS;

#ifdef SLOT_STEALING
// if this is a rxsync slot, skip everything in the slot stealing section
//if (cur_slot.priority != 1)
if (cur_slot.type != TDMA_RXSYNC)
{
...
*/
    ...
    // to notify potential sender and deter any losers.
        ...
        //if (heard_sender!= 1 && cur_slot.priority!=1) 
        if (heard_sender!= 1 && cur_slot.type != TDMA_RXSYNC)
        {
            ...
            // in this cycle
            #ifdef TDMA_SLEEP_OPT
                tdma_sleep_opt_slot = cur_slot.slot;
            #endif
        ...
        // removed because the if just below ifdef slot_stealing takes care of it
        //if (cur_slot.priority!=1)
            rf_tx_set_serial();
            ...
            while (_nrk_high_speed_timer_get() < (DATA_SLOT_START_TICKS+DELTA_SLOT_OFFSET_TICKS))
            {
        ...
        nrk_high_speed_timer_wait(0, DATA_SLOT_START_TICKS + DELTA_SLOT_OFFSET_TICKS);
...
#if defined(TDMA_STATS_COLLECT) && defined(SYNC_HIST)
            sync_rx_log();
#endif
...
#else
            nrk_high_speed_timer_wait(0, SFD_TO_SLOT_TIME-DELTA_SLOT_OFFSET_TICKS);
#endif
...
void _tdma_tx (uint16_t slot)
{
    ...
    //if (cur_slot.type == TDMA_TX_CHILD)
    if (cur_slot.type == TDMA_TXSYNC)
    {
        explicit_tsync=1;
...
#ifdef TDMA_STEAL_UPSTREAM
    // if I am a parent who is going to transmit a sync packet
    if (cur_slot.type == TDMA_TXSYNC)
    {
        // I must transmit during the first delta slot


        // just use a regular jam for now
        rf_test_mode();
        rf_carrier_on();
        ...
        // fool the rest of the slot into thinking it's the beginning
        // when we've really offset it by 1 delta slot
        _nrk_high_speed_timer_reset();
...
//if (cur_slot.type == TDMA_TX_PARENT)
//if (cur_slot.type == TDMA_TXDATA)
// if this isn't a tx-sync, we do the delta slot scheme
if (cur_slot.type!=TDMA_TXSYNC)
{

    // If it's currently past my delta slot already, I can't transmit.
    //if (nrk_time_cmp(current_time, next_delta_offset_time) > 0)
    if (_nrk_high_speed_timer_get() > (TDMA_DELTA_SLOT_TICKS * (cur_slot.priority+1)))
    {
        // I'm late!  Forget it
    ...
    if (cur_slot.priority > 0)
    {
        ...
        while (_nrk_high_speed_timer_get() < (TDMA_DELTA_SLOT_TICKS * (cur_slot.priority * 1)));
#ifdef STEALCHECK_DEBUG
        PORTA &= ~BM(STEALCHECK_DEBUG);
        //PORTA &= ~BM(DEBUG_1);
#endif
...
*/
    ...
    //if (cur_slot.type == TDMA_TX_PARENT && cur_slot.priority > 0)
    if (cur_slot.type == TDMA_TXDATA && cur_slot.priority > 0)
#else
...
#endif
    {
       ...
       //nrk_high_speed_timer_wait(0, (DELTA_SLOT_OFFSET + (TDMA_DELTA_SLOT_TICKS * ((cur_slot.priority*2) + 1))));
    ...
    #endif
    #ifdef TDMA_TEXT_DEBUG
           printf("noack %d p%d\r\n", cur_slot.slot, cur_slot.priority);
    #endif
            nrk_led_set(GREEN_LED);
    #ifdef TDMA_PREAMBLE_HANDSHAKE
        // MOD_CCA 13 Jan 2010
...
#ifdef TDMA_STATS_COLLECT
    stats_start_rdo();
    #ifdef TDMA_TIMESTAMP_PACKETS
    // XXX timestamp the packet! this CHANGES THE DATA IN THE USER-SPECIFIED PACKET!
    if (cur_slot.type != TDMA_TXSYNC)
        tdma_stats_ts_packet(tdma_rfTxInfo.pPayload);
    #endif
#endif
...
{
    printf("tdma M t%d n%d\r\n",nrk_get_pid(), tdma_mac_get());
}
else
{
    printf("tdma S t%d n%d\r\n",nrk_get_pid(), tdma_mac_get());
}
...
if (tdma_schedule_method == TDMA_SCHED_TREE)
{
    ...
    #ifdef TDMA_STATS_COLLECT
        // record the CPU time used up to tree creation, not total system time. D'oh.
        tdma_stats_record_tree_creation();
...
{
    in_sync = 1;
    cur_slot = tdma_schedule_get_next(0);
    ...
    stats_stop_rdo();
                ...
                //cur_slot = tdma_schedule_get_next(sync_slot);
                ...
                cur_slot = tdma_schedule_get_next(sync_slot);
#ifdef TDMA_STATS_COLLECT
                if (nrk_time_sub(&oos_time, sync_time, oos_time) != NRK_ERROR)
                {
                printf("SYNC %lu %lu %lu %lu\r\n",
                    sync_time.secs, sync_time.nano_secs, oos_time.secs, oos_time.nano_secs);

                nrk_time_compact_nanos(&oos_time);
                tdma_stats_add_osa(oos_time);
                }
#endif
                nrk_led_clr(RED_LED);
                in_sync = 1;
...
//#ifdef TDMA_SYNC_DEBUG
                ...
                nrk_time_get(&current_time);
                nrk_time_compact_nanos(&current_time);
                oos_time = current_time;
                printf("OOS %d %d %lu %lu\r\n",
                        my_addr16, oos_count,
                        oos_time.secs, oos_time.nano_secs);
#endif
        ...
        //                            cur_slot.slot, sync_slot, sync_time);

        time_of_slot = gnw_relative(time_of_slot, tdma_slot, cur_slot.slot);
        nrk_time_compact_nanos(&time_of_slot);

        //time_of_slot.nano_secs +=  (uint32_t)5 * (uint32_t)16601 * (uint32_t) NANOS_PER_US;
        //nrk_time_compact_nanos(&time_of_slot);

        //printf("%d -> %d, nw %lu %lu\r\n", tdma_slot, cur_slot.slot, time_of_slot.secs, time_of_slot.nano_secs);
#ifdef GPIO_SLOT_DEBUG
        nrk_gpio_clr(NRK_DEBUG_0);
#endif
...
// THIS IS JUST SOMETHING I'M TRYING OUT 
    ...
    if (tdma_slot < cur_slot.slot)
    {
        slots_to_wakeup = (cur_slot.slot - tdma_slot);
    }
    else
    {
        slots_to_wakeup =  (TDMA_SLOTS_PER_CYCLE - tdma_slot) + cur_slot.slot;
    }
...
*/
    ...
    //cur_slot.type = 50;
        ...
        //if (cur_slot.type == TDMA_RX) // RX
        if (is_rx_slot(cur_slot.type))
        {
#ifdef TDMA_STEAL_UPSTREAM
            // if we're dealing with a sync slot in this configuration,
            // and the normal behavior for this slot is for me to wait for a sync,
            // we will wait to make sure the master is sending a sync packet
            // and if not, we may try to steal the slot
            can_steal = 0;
            if (is_sync_slot(cur_slot.type))
            {
            ...
            PORTA &= ~BM(TDMA_UPSTREAM_DEBUG);
                ...
                // fool slot into thinking it started
                _nrk_high_speed_timer_reset();
            }
            ...
            if (can_steal)
            {
                _tdma_tx(cur_slot.slot);
                if(!tdma_tx_data_ready)
                    nrk_kprintf(PSTR("STOLESYNC\r\n"));
                else
                    nrk_kprintf(PSTR("SSTRY\r\n"));
                    
            }
            else
                _tdma_rx(cur_slot.slot);

#ifdef TDMA_STATS_COLLECT
            if (cur_slot.type == TDMA_RXSYNC && !can_steal) 
            {
                // this is a sync slot that I should've received a sync packet
                sync_slot_cnt++;
            }
#endif
...
*/
        ...
        //else if (tdma_node_mode == TDMA_MASTER && (cur_slot.type == TDMA_TX_CHILD))
        else if (tdma_node_mode == TDMA_MASTER && (cur_slot.type == TDMA_TXSYNC))
        {
            // time to send a sync packet
            // printf("Master send, time token is %d, slot is %d\r\n", tdma_time_token, cur_slot.slot);

            // do we send a sync this cycle?
            tx_sync = !(sync_cycle = (sync_cycle + 1) % SYNC_STRIDE);
            ...
            if (tx_sync)
                _tdma_tx(cur_slot.slot);
#ifdef TDMA_STEAL_UPSTREAM
            // If I'm not transmitting a sync, I must listen for all the delta slots
            // for an incoming packet.
            else
                _tdma_rx(cur_slot.slot);

            // FIXME This is not working!  Make it work! (it's a timing problem I think)

            if (tdma_rx_data_ready)
                nrk_kprintf(PSTR("SYNCDATA!\r\n"));
            //else
            //    nrk_kprintf(PSTR("NOSDATA\r\n"));
#endif
        ...
        else if (tdma_node_mode == TDMA_SLAVE)
        {
            ...
            if (cur_slot.type == TDMA_TXSYNC)
            {
                // TODO: Implement the slot stealing listening for here, the sync slot stealing ('triple-s' opt?)

                // whether to TX sync depends on whether I received a packet before
                // (which can be taken care of in the RX func by setting tx_sync to 1)
                // but also if I haven't sent one recently I should send
                ...
                if (tx_sync)
                {
                    ...
                    tdma_rfTxInfo.length = TDMA_DATA_START;
                    _tdma_tx(cur_slot.slot); // send sync
            ...
            //else if (cur_slot.type==TDMA_TX_PARENT)
            else if (cur_slot.type==TDMA_TXDATA)
            {
                if (tdma_tx_data_ready)
                {
                    // send a real packet
        ...
        cur_slot = tdma_schedule_get_next(cur_slot.slot);
            ...
            if (tdma_sleep_opt_slot != 0xFFFF)
            {
                ...
                //while(cur_slot.type == TDMA_RX
                while(cur_slot.type == TDMA_RXDATA
                      //&& cur_slot.priority != 1
                      // and the slot is further along in the cycle
                      && cur_slot.slot > tdma_sleep_opt_slot)
                {
                    cur_slot = tdma_schedule_get_next(cur_slot.slot);
                }
            
                if (cur_slot.slot <= tdma_sleep_opt_slot)
                    tdma_sleep_opt_slot = 0xFFFF;
            }
        #endif
