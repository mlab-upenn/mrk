...
void rx_task ()
{
  ...
  t.secs=600;
  t.nano_secs=0;
  // Setup software watchdog timer 0 to expire every 600 seconds and reboot
  val=nrk_sw_wdt_init(GW_RX_TASK_SWDT_ID, &t, NULL);
  if(val==NRK_ERROR)
	nrk_kprintf( PSTR("Error creating RX task SWDT\r\n" ));

  nrk_sw_wdt_start(GW_RX_TASK_SWDT_ID);
  ...
  while (1) {
    nrk_sw_wdt_update(GW_RX_TASK_SWDT_ID);
...
void tx_task ()
{
  ...
  check_period.secs=600;
  check_period.nano_secs=0;
  len=nrk_sw_wdt_init(GW_TX_TASK_SWDT_ID, &check_period, NULL);
  if(len==NRK_ERROR)
	nrk_kprintf( PSTR("Error creating RX task SWDT\r\n" ));

  nrk_sw_wdt_start(GW_TX_TASK_SWDT_ID);
  ...
  while (1) {
   ...
   nrk_sw_wdt_update(GW_TX_TASK_SWDT_ID);
   error=1;
