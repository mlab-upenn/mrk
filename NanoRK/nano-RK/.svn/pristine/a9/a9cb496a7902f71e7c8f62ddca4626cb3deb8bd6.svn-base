...
//    ping_p2p_generate(&p2p_pkt);
    ...
    if( (p2p_pkt.ctrl_flags & LINK_ACK)!=0 && p2p_pkt.dst_mac!=255 ) 
	{
	uint8_t retry;
	retry=0;
	 do {
    ...
    while (1) {
      ...
      if (my_sigs & SIG (rx_signal)) {
	    ...
	    printf( "cnt: %lu ",ping_cnt );
            printf ("mac: %d %d %d %d ", p2p_pkt.src_subnet_mac[2],
                    p2p_pkt.src_subnet_mac[1], p2p_pkt.src_subnet_mac[0],
		    p2p_pkt.src_mac);
