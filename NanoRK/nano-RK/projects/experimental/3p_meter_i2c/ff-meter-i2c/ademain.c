...
int main ()
{
  ...
  V_RdPtr_U32R = 0;
  slip_tx (&V_TxBuff_U8R[V_RdPtr_U32R], C_SlipPktSize,
           &V_TxBuff_U8R[C_CircBuffSize - 1], C_CircBuffSize);
...
////////////////////////////
  ...
  if (V_EvnDetCntr_U32R == Fs) {
    ...
    if ((V_DiffData_S32R > C_ThresholdPwr)
        || (V_DiffData_S32R < (-1 * C_ThresholdPwr))) {
      F_Occ_U8R = 0;
      F_Det_U8R = 0;
    }
    else {
...
/////////////////////////////
  if (F_ReadyForSignal_U8R == 1) {
    if (((V_WrSeqNo_U8R * C_CircBuffSize + V_WrPtr_U32R) - (V_RdPtr_U32R)) >=
        C_SlipPktSize) {
