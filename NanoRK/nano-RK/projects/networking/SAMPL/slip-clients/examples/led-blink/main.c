...
retry_num=0;
  ...
  if (argc < 4 ) {
    ...
    printf ("\nExample Usages:\n");
    printf ("Blink LED on node 00000a04 with gateway_cleint running\n");
    printf ("  %s localhost 5001 0x00000a04\n",argv[0]);
    printf ("Blink LED on all nodes in 00000a subnet\n");
    printf ("  %s localhost 5001 0x00000aff\n",argv[0]);
    exit (1);
  }
...
for(retry_cnt=0; retry_cnt<retry_num; retry_cnt++ )
{
    ...
    tran_pkt.checksum = 0;
		...
		tran_msg.payload[0]=TRAN_RED_LED_MASK;
